//! The webrtc voice engine: peer-connection setup, cpal mic/speaker streams, and
//! the capture/render audio processor (AEC/NS/AGC + Opus). Platform-neutral; the
//! desktop and android clients both drive this. Signaling transport (the grpc
//! voice stream) lives in each client; this layer only sends/receives domain
//! `ClientVoice`/`ServerVoice` through [`WebRTCConnection`].

use crate::mixer::Mixer;
use crate::resampler::Resampler;
use chat_shared::convert::IntoProto;
use chat_shared::convert::stream::proto::ClientVoiceMessage;
use chat_shared::domain::stream::ClientVoice;
...
/// Outbound signaling channel: the voice engine sends domain `ClientVoice`
/// messages, which each client forwards over its own grpc voice stream. The
/// client builds this from the stream's sender via [`WebRTCConnection::new`].
#[derive(Debug, Clone)]
pub struct WebRTCConnection(mpsc::Sender<ClientVoiceMessage>);

impl WebRTCConnection {
  pub fn new(sender: mpsc::Sender<ClientVoiceMessage>) -> Self {
    Self(sender)
  }
